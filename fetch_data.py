...
def main():
    ...
    def prev_close(s):
        q=Q.get(s,{})
        # Angel One returns previous close in different fields
        for field in ['previousClose','prevClose','lastTradePrice']:
            v=float(q.get(field,0) or 0)
            if v>0: return round(v,2)
        return 0.0
    ...
    # Debug — print raw quote for NIFTY50 and VIX
    nifty_q = Q.get("NIFTY50",{})
    vix_q = Q.get("INDIAVIX",{})
    fn_q = Q.get("FINNIFTY",{})
    print(f"NIFTY50 raw: ltp={nifty_q.get('ltp')} close={nifty_q.get('close')} prevClose={nifty_q.get('previousClose')}")
    print(f"VIX raw: ltp={vix_q.get('ltp')} close={vix_q.get('close')} prevClose={vix_q.get('previousClose')}")
    print(f"FINNIFTY raw: ltp={fn_q.get('ltp')} close={fn_q.get('close')} prevClose={fn_q.get('previousClose')}")
