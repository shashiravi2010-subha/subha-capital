...
def main():
    ...
    def open_px(s):
        q=Q.get(s,{})
        return float(q.get("open",0) or px(s))

    nifty_px=px("NIFTY50"); nifty_chg=chg("NIFTY50")
    vix_px=px("INDIAVIX")
    fn_px=px("FINNIFTY"); fn_chg=chg("FINNIFTY")
    print(f"Nifty: {nifty_px} ({nifty_chg}%) | VIX: {vix_px} | FINNIFTY: {fn_px} ({fn_chg}%)")

    # FINNIFTY ATM Strike (nearest 50)
    fn_atm = round(fn_px/50)*50 if fn_px>0 else 0

    # FINNIFTY 30-min ORB candle data
    fn_orb = {"orb_open":0,"orb_high":0,"orb_low":0,"orb_close":0,"signal":"WAIT"}
    try:
        fn_date = ist.strftime("%Y-%m-%d")
        fn_candles = get_candles(jwt,"26037","THIRTY_MINUTE",
                                  f"{fn_date} 09:00",f"{fn_date} 10:00")
        if fn_candles.get("status") and fn_candles.get("data"):
            c = fn_candles["data"]
            if c:
                # First 30-min candle = 9:15 to 9:45 AM
                first = c[0]
                ...
                # Signal logic
                if fn_px > fn_orb["orb_high"] and fn_orb["orb_high"]>0:
                    fn_orb["signal"] = "CE"
                elif fn_px < fn_orb["orb_low"] and fn_orb["orb_low"]>0:
                    fn_orb["signal"] = "PE"
                else:
    ...
    # FINNIFTY Options premium (ATM CE and PE)
    # Token format: FINNIFTY + expiry + strike + CE/PE
    # Using approximate premium calculation if API unavailable
    fn_ce_premium = 0
    fn_pe_premium = 0
    try:
        # Try fetching ATM options
        # FINNIFTY options tokens vary by expiry — approximate for now
        # Will be enhanced with actual options chain
        if fn_px > 0:
            vix_factor = max(vix_px,12) if vix_px>0 else 14
            days_to_expiry = 2  # Tuesday expiry approx
            fn_ce_premium = round(fn_px * (vix_factor/100) * (days_to_expiry/365)**0.5 * 100, 0)
            fn_pe_premium = round(fn_ce_premium * 0.95, 0)  # PE slightly lower
    except: pass
    ...
    finnifty_data = {
        ...
        "chg": fn_chg,
        "atm": fn_atm,
        "orb_open":  fn_orb["orb_open"],
        "orb_high":  fn_orb["orb_high"],
        "orb_low":   fn_orb["orb_low"],
        "orb_close": fn_orb["orb_close"],
        "signal":    fn_orb["signal"],
        "ce_premium": fn_ce_premium,
        "pe_premium": fn_pe_premium,
    ...
    print(f"FINNIFTY: ATM={fn_atm} ORB H={fn_orb['orb_high']} L={fn_orb['orb_low']} Signal={fn_orb['signal']}")

    # Sectors
    sectors=[]
    for sec,stks in SECTOR_STOCKS.items():
        chgs=[chg(s) for s in stks if px(s)>0]
        sec_chg=round(sum(chgs)/len(chgs),2) if chgs else 0.0
        ldr=stks[0]
        sectors.append({"name":sec,"chg":sec_chg,"leader":ldr,
                        "leader_price":px(ldr),"leader_chg":chg(ldr),"idx_price":0,
    ...
    data={
        ...
        "is_live":True,
        "market":{"nifty50":{"ltp":nifty_px,"chg":nifty_chg},
                  "vix":{"ltp":vix_px,"chg":chg("INDIAVIX")},"bias":bias},
