...
def fetch_gift_nifty():
    """Fetch Gift Nifty from public API"""
    gift = {"ltp":0,"chg":0,"gap":0,"gap_pct":0,"bias":"—","source":"—"}
    try:
        hdrs_yf={"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
                 "Accept":"application/json","Accept-Language":"en-US,en;q=0.9"}
        # Try Yahoo Finance for Nifty (as proxy for Gift Nifty direction)
        for sym in ["^NSEI","^CNXNIFTY"]:
            try:
                r=requests.get(f"https://query1.finance.yahoo.com/v8/finance/chart/{sym}?interval=1d&range=1d",
                               headers=hdrs_yf,timeout=8)
                if r.ok:
                    d=r.json()
                    ...
                    if result:
                        ...
                        if ltp>0 and prev>0:
                            ...
                            gift["ltp"]=round(ltp,2)
                            gift["chg"]=chg
                            gift["source"]="Yahoo Finance"
                            break
            except: continue
        ...
        if gift["ltp"]>0:
            ...
            if gift["chg"]>0.3:   gift["bias"]="BULLISH GAP UP ▲"
            elif gift["chg"]<-0.3: gift["bias"]="BEARISH GAP DOWN ▼"
